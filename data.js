...
//   // cardNo.innerHTML = data[i].id;
//   cardTitle.append(data[i].name)
//   cardText.append(data[i].email);
// }
...
next.addEventListener("click", () => {
  ...
  page == data.length - 6 ? (page = 0) : (page += 6);
  row.innerHTML = "";
  for (idx = page; idx < page + 6; idx++) {
    const card = document.createElement('div');
    card.classList = 'card-body';
    let result = data[idx];
    const content =
...
//   // Create card element
//   const card = document.createElement('div');
//   card.classList = 'card-body text-center offset-sm-3 col-sm-6 col-12';
